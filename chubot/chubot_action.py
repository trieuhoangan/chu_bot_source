...
class ChuBotAction():
    ...
    def load_domain(self, domain_file):
        ...
        with io.open(domain_file) as f:
            action_domain = json.loads(open(domain_file,encoding="utf-8").read())
    ...
    def handle_action(self, action, **kwargs):
        ...
        if action in self.action_templates:
            ...
            func = getattr(custom_actions, self.action_custom[action].get("func_name"))
            response = func(action_args = self.action_custom[action].get("kwargs"), **kwargs)
